...
function prepareDataForSheet(balancesDeEstaEntidad, cuentasMap, nominaMap, allMonths, indicesMap, num_entidad) {
    ...
    dataForSheet.push([ 'MUY IMPORTANTE: Se debe elegir sólo un año completo y sólo uno, desde enero a diciembre del mismo año. Si has eligido más de un año, para las cuentas que han tenido saldo sólo en el primer año seleccionado y que en el año subsiguiente ya han dejado de tener saldo, la aplicación arrastrará un saldo incorrecto para los años en que han dejado de tener movimiento. También arrastra, incorrectamente, el ajuste por inflación acumulado en el año anterior, al año subsiguiente (porque no tiene definida la "refundición de cuentas". Falta corregir tales errores en la lógica del prógrama. Seleccionando sólo un año calendario, tales errores NO se producirán, en absoluto.']);
    dataForSheet.push(['Para las cuentas que comienza con 62, que son las cuentas dónde se expone el resultado monetario generado por el ajuste por inflación, estas cuentas - debido a su particularidad - no tienen saldo histórico, pero la applicación muestra incorrectamente un saldo histórico. Falta corregir dicho error en la lógica del programa.']);
    ...
    dataForSheet.push(['Dado que el server sólo permite subir archivos de datos hasta un cierto límite máximo de tamaño, el archivo tamaño del archivo de balances txt ha sido reducido. estándo sólo disponibles los balances desde Enero 2022 en adelante. Si necesitaras años anteriores, solo tienes que avisarle a Vincenzo y él - volentieri - proveerá.']);
